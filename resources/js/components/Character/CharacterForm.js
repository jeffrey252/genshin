...
const CharacterForm = ({ history, match }) => {
  ...
  function addCharacter() {
    CharacterService.create(character).then((response) => {
  ...
  function updateCharacter() {
    CharacterService.update(character).then((response) => {
      alertService.sendAlert({
        type: "success",
      ...
      <div className="card-body">
        <Form onSubmit={saveCharacter}>
          <Form.Group className="mb-3" controlId="characterForm.ControlInput1">
            ...
            <Form.Control
              as="select"
              name="talent_material_id"
              value={character.talent_material_id}
              onChange={handleInputChange}
            >
              <option value="" disabled>
                Select talent material
              </option>
              {talentMaterials &&
                talentMaterials.map((talentMaterial) => (
                  <option key={talentMaterial.id} value={talentMaterial.id}>
                    {talentMaterial.name} ( {talentMaterial.schedule.name} ){" "}
                  </option>
                ))}
            </Form.Control>
